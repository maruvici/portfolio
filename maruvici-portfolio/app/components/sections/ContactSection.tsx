...
import { ContactLink } from "@/data/personal";
  ...
  return (
    <section id="contact">
      ...
      <div className="sec-label">
        <span className="sec-label-prefix">~/</span>contact.sh
      </div>
      <h2 className="sec-title">Get In Touch</h2>

      {/* Contact cards */}
      <div className="contact-grid">
        {personal.contact.map((link) => (
          <a
            key={link.type}
          ...
          >
            <div className="contact-icon">{ICONS[link.type]}</div>
            <div>
              <div className="contact-label">{link.label}</div>
              <div className="contact-val">{link.value}</div>
            </div>
          </a>
        ))}
      </div>
